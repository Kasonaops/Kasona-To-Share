...
> **Purpose:** Take a cloned website template and systematically re-brand it for a new client by swapping six core elements: **Logo Â· Copy Â· Trusted Brands Â· Pricing Â· Reviews Â· Colour Scheme**.
...
The cloned template is a **Vite + React + TypeScript** project. Below is a precise map of which files control which brandable element. Always verify paths by running `list_dir` first â€” the repo structure may have been reorganised.
...
| 3 | **Trusted Brands** | `components/SocialProof.tsx`                                                                                                                                       | Brand name strings in the `brands` array â€” replace with real client logos/names                                                                                                                                                                                           |
...
| 6 | **Colour Scheme**  | **ALL components** + `components/ui/*` + `index.html` `<style>` block                                                                                    | Primary accent (`#BFF549`), gradient mid-points, background (`#02040a`), muted text (`#99A1AF`), button glows, shadow rgba values, volumetric effects, icon fills, border colors, selection color, scrollbar colors â€” **requires component-by-component audit** |
...
- [ ] **Logo image file** â€” request in these dimensions:
  - Navbar icon: **40 Ã— 40 px** (PNG or SVG, transparent background)
  - Footer icon: **32 Ã— 32 px** (PNG or SVG, transparent background)
  - Favicon: **32 Ã— 32 px** (ICO or PNG)
...
components/Navbar.tsx  â†’  logo icon + "VOLTFLOW" text
components/Footer.tsx  â†’  logo icon + "VOLTFLOW" text
index.html             â†’  `<title>` tag + favicon link

```

### Phase 2 â€” Copy Changes
...
- [ ] **Hero sub-headline / tagline** (replaces "Elevating digital presenceâ€¦")
...
- [ ] **Pricing section sub-text** (e.g., "Clear, transparent pricingâ€¦")
- [ ] **Testimonials section headline**
- [ ] **Footer tagline** (replaces "We help elite businesses dominateâ€¦")
...
### Phase 3 â€” Trusted Brands Logos

Ask the user for the following:

- [ ] **List of brand names** to display in the marquee (replaces TechVibe, NexusLabs, etc.)
- [ ] **Brand logo images** (optional) â€” request in these dimensions:
  - Marquee logos: **height 48â€“64 px**, width proportional, **SVG preferred** (PNG with transparent background also acceptable)
...
components/SocialProof.tsx  â†’  brands array / logo images + label text
...
components/Pricing.tsx  â†’  pricingPlans array + section heading + CTA labels

```

### Phase 5 â€” Reviews / Testimonials
...
- [ ] For **each testimonial**, collect:
  ...
  - Star rating (1â€“5)
  - Avatar image â€” request in these dimensions:
    - **100 Ã— 100 px** (PNG or JPG, square, ideally circular crop)
...
components/Testimonials.tsx  â†’  testimonials array

```

### Phase 6 â€” Colour Scheme

Ask the user for the following:

- [ ] **Primary accent colour** (replaces `#BFF549` â€” the lime green used for CTAs, highlights, stars, badges)
- [ ] **Background colour** (replaces `#02040a` â€” the near-black base)
- [ ] **Muted text colour** (replaces `#99A1AF` â€” used for secondary / descriptive text)
- [ ] **Secondary accent** (optional â€” replaces the blue/purple accents in Features cards)
...
- [ ] If the user provides a single brand colour, **derive the full palette automatically**:
  - Primary accent â†’ user colour
  - Mid-tone for gradients â†’ lighten/shift hue 15%
  - Hover states â†’ lighten 10%
  - Glow/blur overlays â†’ same hue at 5â€“10% opacity
  - Selection â†’ accent bg + black text

**CRITICAL â€” Color appears in many places beyond simple backgrounds:**
...
#BFF549   â†’  new primary accent     (ALL files - use replace_all: true)
#FACC15   â†’  new gradient mid       (if present - secondary yellow/gold)
#02040a   â†’  new background         (ALL files + index.html `<style>`)
#99A1AF   â†’  new muted text         (ALL files)
rgba(191,245,73,...)  â†’  new rgba   (shadow/glow effects)
index.html            â†’  ::selection, scrollbar, body background
...
- [ ] Perform a **global find-and-replace** across all files for each color token:
  - [ ] Replace `#BFF549` (or old primary) â†’ new primary (hex uppercase)
  - [ ] Replace `#FACC15` (if present) â†’ new gradient mid-point
  - [ ] Replace `rgba(191,245,73,X)` â†’ new rgba with same alpha
  - [ ] Replace `#02040a` â†’ new background
  - [ ] Replace `#99A1AF` â†’ new muted text
...
- [ ] Update `Navbar.tsx` â€” swap icon component / add `<img>` tag, update brand name text.
- [ ] Update `Footer.tsx` â€” same as above.
- [ ] Update `index.html` â€” `<title>` tag, add `<link rel="icon">` if favicon provided.

### Step 4 â€” Copy
- [ ] Update `Hero.tsx` â€” headline, sub-headline, badge, CTAs.
- [ ] Update `Navbar.tsx` â€” nav links, CTA button label.
- [ ] Update `Features.tsx` â€” section title, card titles, descriptions.
- [ ] Update `Pricing.tsx` â€” section headline, sub-text (if different from pricing data).
- [ ] Update `Footer.tsx` â€” tagline, copyright, column titles/links.
- [ ] Update `index.html` â€” `<title>`, meta description.

### Step 5 â€” Trusted Brands
- [ ] Update `SocialProof.tsx` â€” replace `brands` array with new names or `<img>` logos.
- [ ] If using images, save to `/public/brands/` and reference from component.

### Step 6 â€” Pricing
- [ ] Update `Pricing.tsx` â€” replace `pricingPlans` array with new data.

### Step 7 â€” Reviews
- [ ] Update `Testimonials.tsx` â€” replace `testimonials` array with new data.
...
|-------|-----------|--------|-------|
| Navbar logo icon | 40 Ã— 40 px | SVG or PNG (transparent) | Square, used in rounded container |
| Footer logo icon | 32 Ã— 32 px | SVG or PNG (transparent) | Matches navbar but smaller |
| Favicon | 32 Ã— 32 px | ICO or PNG | Browser tab icon |
| Trusted brand logos | height 48â€“64 px | SVG preferred | Used in scrolling marquee |
| Testimonial avatars | 100 Ã— 100 px | PNG or JPG | Square, displayed as circle |
...
- **Common patterns:**
  - Purple gradient: `#7C3AED â†’ #A855F7 â†’ #7C3AED`
  - Blue gradient: `#3B82F6 â†’ #60A5FA â†’ #3B82F6`
  - Green gradient: `#10B981 â†’ #34D399 â†’ #10B981`

---

## Error Handling

- If `npm install` fails â†’ check Node.js version (`node -v`, needs â‰¥ 18).
...
- If logo images don't load â†’ verify paths are relative to `/public/` and referenced from root (`/logo.svg`).
...
- If the user provides only some of the six elements â†’ implement what's provided, leave the rest as template defaults, and note which items still need customization.
- If TypeScript errors appear after changes â†’ these are often just type hint issues and won't prevent the site from running in dev mode. Focus on visual verification in the browser.

---

## Interaction Style

- Be **systematic** â€” work through phases 1â€“6 in order.
- Be **specific** â€” when asking for assets, always state the exact dimensions and preferred format.
- Be **efficient** â€” ask for all items within a phase in a single message, not one at a time.
- Be **confirmatory** â€” before implementing, summarise all collected information and get a final "go" from the user.
- **Never assume** â€” if the user hasn't provided something, ask. Don't invent brand names, pricing, or testimonials.
